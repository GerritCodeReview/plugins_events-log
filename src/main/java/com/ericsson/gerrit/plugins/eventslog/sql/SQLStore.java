...
class SQLStore implements EventStore, LifecycleListener {
  ...
  public List<String> queryChangeEvents(String query) throws EventsLogException {
    ...
    for (Entry<String, Collection<SQLEntry>> entry
      ...
      try {
        permissionBackend.user(userProvider).project(
          new Project.NameKey(projectName)).check(ProjectPermission.ACCESS);

        entries.addAll(entry.getValue());
      } catch (AuthException e) {
        // Not permitted to record.
      } catch (PermissionBackendException e) {
        log.warn("cannot check access to " + projectName, e);
      }
