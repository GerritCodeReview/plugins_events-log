...
public class EventsLogConfigTest {
  ...
  private void setUpDefaults() {
    when(configMock.getBoolean(CONFIG_COPY_LOCAL, DEFAULT_COPY_LOCAL)).thenReturn(DEFAULT_COPY_LOCAL);
    ...
    when(configMock.getInt(CONFIG_RETURN_LIMIT, DEFAULT_RETURN_LIMIT)).thenReturn(DEFAULT_RETURN_LIMIT);
    ...
    when(configMock.getString(CONFIG_LOCAL_PATH, defaultLocalStorePath)).thenReturn(defaultLocalStorePath);
    ...
    when(configMock.getInt(CONFIG_EVICT_IDLE_TIME, DEFAULT_EVICT_IDLE_TIME)).thenReturn(DEFAULT_EVICT_IDLE_TIME);
  ...
  private void setUpCustom() {
    ...
    when(configMock.getString(CONFIG_URL, DEFAULT_URL)).thenReturn("jdbc:h2:~/gerrit/db");
    ...
    when(configMock.getInt(CONFIG_EVICT_IDLE_TIME, DEFAULT_EVICT_IDLE_TIME)).thenReturn(CUSTOM_EVICT_IDLE_TIME);
