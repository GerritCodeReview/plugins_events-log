...
public class EventsLogCleaner implements ProjectDeletedListener {
  ...
  public void scheduleCleaningWith(int maxAge, long initialDelay, long interval) {
    pool.scheduleAtFixedRate(
        () -> eventsDb.removeOldEvents(maxAge), initialDelay, interval, TimeUnit.MILLISECONDS);
    pool.scheduleAtFixedRate(
        () -> localEventsDb.removeOldEvents(maxAge), initialDelay, interval, TimeUnit.MILLISECONDS);
  }
