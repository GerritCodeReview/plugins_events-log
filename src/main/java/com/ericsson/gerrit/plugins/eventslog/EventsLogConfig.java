...
public class EventsLogConfig {
  ...
  EventsLogConfig(PluginConfigFactory cfgFactory, SitePaths site, @PluginName String pluginName) {
    ...
    storeUrl = cfg.getString(pluginName, null, CONFIG_URL);
    if (storeUrl == null) {
      storeUrl = H2_DB_PREFIX + site.data_dir.resolve("db").normalize();
    }
    String stringPath = cfg.getString(pluginName, null, CONFIG_LOCAL_PATH);
    if (stringPath == null) {
      stringPath = site.resolve("events-db").normalize().toString();
    }

    localStorePath = Paths.get(stringPath);
