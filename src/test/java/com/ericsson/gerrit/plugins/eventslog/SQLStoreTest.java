...
import static com.ericsson.gerrit.plugins.eventslog.SQLStore.H2_DB_SUFFIX;
...
public class SQLStoreTest {
  ...
  public void setUp() throws SQLException, IOException {
    localUrl = "jdbc:h2:" + testFolder.toString();
    testFile = testFolder.newFile(TABLE_NAME + H2_DB_SUFFIX);
    testCopyFile = testFolder.newFile(TABLE_NAME + "copy" + H2_DB_SUFFIX);
    ...
    Path localPath = easyMock.createNiceMock(Path.class);
    Path localCopyPath = easyMock.createNiceMock(Path.class);
    cfgMock = easyMock.createNiceMock(EventsLogConfig.class);
    expect(cfgMock.getMaxAge()).andReturn(5);
    expect(cfgMock.getLocalStorePath()).andReturn(localPath).atLeastOnce();
    expect(localPath.resolve(TABLE_NAME + H2_DB_SUFFIX)).andReturn(localPath).anyTimes();
    expect(localPath.resolve(EasyMock.anyString())).andReturn(localCopyPath).anyTimes();
    expect(localPath.toFile()).andReturn(testFile).atLeastOnce();
    expect(localCopyPath.toFile()).andReturn(testCopyFile).anyTimes();
  }
  ...
  private void setUpClient() {
    ...
    localEventsDb = new SQLClient(TEST_DRIVER, localUrl, TEST_OPTIONS);
    store =
  ...
  private void setUpClientMock(boolean reset) {
